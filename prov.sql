...
--проверка на то что судно на которое покупается билет не похищено пиратами и не в утиле--
      IF NOT ( SELECT 1 FROM ship where ((id_ship=NEW.id_ship and status='swim') or (id_ship=NEW.id_ship and status='in port'))) THEN
       RAISE EXCEPTION 'Наше судно не может сейчас совершать рейсы';
      END IF;
--проверить что на рейсе свободных мест больше 0
      IF NOT (SELECT 1 FROM flight WHERE (id_flight=NEW.id_flight and quantity_empty_place>0)) THEN
...
--проверить что наш корабль участвует в желаемом нами рейсе и свободных кабин нужного типа больше 0--
      IF NOT (SELECT 1 FROM ship_for_flight WHERE (id_ship=NEW.id_ship and id_flight=NEW.id_flight and   ((quantity_empty_place_lux>0 and NEX.class_ticket='lux') or (quantity_empty_place_stand>0 and NEX.class_ticket='standart') ))) THEN
       RAISE EXCEPTION 'Наше судно не участвуют в этом рейсе или не осталось свободных мест';
      END IF;
      
      --так мы знаем номер кабины и нужный рейс,  проверим по билетам и пересечениям портов, если таких нет то производим вставку--
     IF EXISTS (SELECT 1 FROM ticket WHERE (id_cabin=NEW.id_cabin AND id_flight=new.id_flight AND ((id_port_from>NEW.id_port_from and id_port_from<NEW.id_port_where) or (id_port_where>NEW.id_port_from and id_port_where<NEW.id_port_where) or (id_port_from=NEW.id_port_from and id_port_where=NEW.id_port_where) OR (NEW.id_port_from>id_port_from and NEW.id_port_from<id_port_where) and (id_port_from<id_port_where) and (NEW.id_port_from<NEW.id_port_where))))
      THEN
...
IF ((NEW.id_port_where-NEW.id_port_from)*(select price_between from cabin where id_cabin=NEW.id_cabin)!=NEW.price) THEN
...
--ЕСЛИ У НАС СДАЮТ БИЛЕТ--
   ELSIF (TG_OP = 'DELETE') THEN
     UPDATE flight SET (quantity_empty_place, quantity_busy_place) = (quantity_empty_place+1, quantity_busy_place-1)
  WHERE id_flight = OLD.id_flight;--нужный нам корабль: select id_ship from cabin where id_cabin=NEW.id_cabin) --
CREATE OR REPLACE FUNCTION tic() RETURNS TRIGGER AS $$
   BEGIN
...
--проверка совпадения желаемого класса каюты и запрашиваемой каюты--
      ...
      END IF;
--проверка на то что судно на которое покупается билет не похищено пиратами и не в утиле--
      IF NOT ( SELECT 1 FROM cabin where ((select status from ship where id_ship=(select id_ship from cabin where id_cabin=NEW.id_cabin))='swim' or (select status from ship where id_ship=(select id_ship from cabin where id_cabin=NEW.id_cabin))='in port')) THEN
       RAISE EXCEPTION 'Наше судно не может сейчас совершать рейсы';
      END IF;
--проверить что на рейсе свободных мест больше 0
      IF NOT (SELECT 1 FROM flight WHERE (id_flight=NEW.id_flight and quantity_empty_place>0)) THEN
